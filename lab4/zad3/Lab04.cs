...
class Example
    ...
    public static void Main ()
        ...
        // nieskierowany - cykl
        rgg.SetSeed(1111);
        g = rgg.EulerGraph(typeof(AdjacencyMatrixGraph),false,6,2,4);
        ...
        // skierowany - cykl
        rgg.SetSeed(2222);
        g = rgg.EulerGraph(typeof(AdjacencyMatrixGraph),true,5,1,3);
        b=g.Lab04_Euler(out ep);
        if ( b )
            {
        ...
        rgg.SetSeed(3333);
        g = rgg.SemiEulerGraph(typeof(AdjacencyMatrixGraph),false,6,2,4);
        b=g.Lab04_Euler(out ep);
        if ( b )
            {
        ...
        // skierowany - sciezka
        rgg.SetSeed(4444);
        g = rgg.SemiEulerGraph(typeof(AdjacencyMatrixGraph),true,5,1,3);
        b=g.Lab04_Euler(out ep);
        if ( b )
            {
    ...
    public static IGraph Construct(int n, Edge[] ep)
        {
        IGraph g = new AdjacencyMatrixGraph(true,n);
